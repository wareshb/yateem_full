...
export default function OrphansCreate() {
    ...
    useEffect(() => {
        const fetchNextId = async () => {
            try {
                const res = await axios.get(`${API_URL}/orphans/next-id`);
                if (res.data?.next_id) {
                    setFormData(prev => ({ ...prev, orphan_id: res.data.next_id }));
                }
            } catch (err) {
    ...
    return (
        <div className="grid" style={{ gap: 16 }}>
            <div className="section-title">
                <h2>إضافة يتيم جديد</h2>
                <button type="button" className="button secondary" onClick={() => navigate('/orphans')}>
                    إلغاء
                </button>
            </div>
            ...
            <div className="card">
                <form onSubmit={handleSubmit}>
                    ...
                    <Section title="بيانات الأصل">
                        ...
                        <Input label="مديرية الأصل" value={formData.origin_district} onChange={(e) => setFormData({ ...formData, origin_district: e.target.value })} />
                    </Section>
...
function getEmptyForm() {
    return {
        orphan_id: '', // Will be populated by API
        full_name: '',
        date_of_birth: '',
        gender: 'male',
        nationality: 'يمني',

...
export default function SponsorshipsEdit() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [sponsorshipRes, orphansRes, sponsorsRes] = await Promise.all([
                    axios.get(`${API_URL}/sponsorships/${id}`),
                    axios.get(`${API_URL}/orphans`),
                    axios.get(`${API_URL}/sponsors`)
                ]);

                const data = sponsorshipRes.data;
                setFormData({
                    ...data,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await axios.put(`${API_URL}/sponsorships/${id}`, dataToSend);
            alert('تم تحديث الكفالة بنجاح');
            navigate(`/sponsorships/${id}`);
        } catch (err) {
    ...
    return (
        <div className="grid" style={{ gap: 16 }}>
            <div className="section-title">
                <h2>تعديل الكفالة</h2>
                <button type="button" className="button secondary" onClick={() => navigate(`/sponsorships/${id}`)}>
                    إلغاء
                </button>
            </div>
